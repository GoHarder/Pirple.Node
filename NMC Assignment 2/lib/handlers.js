                              ...
                              _data.delete('items', itemId, (err)=>{
                                 ...
                                 if (itemsDeleted == itemsToDelete) {
                                    if (!deletionErrors) {
                                       ...
                                       callback(500, {'Error': 'Error in deleting item'})
                                    }         
                                 }
                              })
                           });
            ...
            _data.read('users', userFile, (err, userData)=>{
               if (!err && userData) {
                  ...
                  if (cartItems.length < config.maxItems ) {
                        ...
                        if (!err) {
                           ...
                           _data.update('users', userFile, userData, (err)=>{
                              if (!err) {
                                 ...
                                 callback(500, {'Error': 'Could not update the user with the new item'});
                              }
                           }); 
                        } else {
                     ...
                     callback(400, {'Error': `The user has the maximum number of items in their cart (${config.maxItems})`});
                  }
               } else {
                  callback(403);
               }
            ...
            _data.read('items', id, (err, readData)=>{
               ...
               handlers._tokens.verifyToken(token, readData.userEmail, (tokenIsValid)=>{
                  if (tokenIsValid) {
                     ...
                     // Create a price for the item
                     var price = config.menuData.size[readData.size].price;

                     readData.toppings.forEach((topping) => {
                        price += config.menuData.toppings[topping].price;
                     });
                        ...
                        _data.read('users', userFile, (err, userData)=>{
                           if (!err && userData) {      
                              ...
                              if (itemPosition > -1) {
                                 ...
                                 callback(500, {'Error': 'Could not find the item in the object'});
                              }  
                           } else {
                              callback(400, {'Error': 'Could not find the specified user'});
                           }
                              ...
                              if (!err) {
                                 ...
                                 handlers._deleteCart(userData, (err, userData)=>{
                                    if (!err && data) {
                                       ...
                                       callback(500, {'Error': 'Could not clean up cart'});
                                    }
                                 });
                              } else {
                                 callback(500, {'Error': 'Send receipt failed'});
                              }
                           });
                        } else {
...
handlers._deleteCart = (userData, callback)=>{
   ...
   // Log the transaction
   var logFileName = `Orders-${new Date().toDateString().split(' ').slice(1).join(' ')}-${_logs.time()}`
   var logString = JSON.stringify(userData);

   // Append new user to current day order log
