         ...
         if (err) {
            ...
            if (hashedPassword) {
               ...
               _data.create('users', userFile, userObject, err => {
                  if (!err) {
                     var logFileName = `Users-${new Date()
                        .toDateString()
                        .split(' ')
                        .slice(1)
                        .join(' ')}-${_logs.time()}`;
                     var logString = JSON.stringify(userObject);

                     // Append new user to current day sign up log
                     _logs.append(logFileName, logString, err => {
                        if (!err) {
                           ...
                           console.log(err);
                           callback(500, {
                              Error: 'Could not log the new user'
                           });
                        }
                     });
                  } else {
...
handlers._deleteCart = (userData, callback) => {
   // Log the transaction
   var logFileName = `Orders-${new Date()
      .toDateString()
      .split(' ')
      .slice(1)
      .join(' ')}-${_logs.time()}`;
