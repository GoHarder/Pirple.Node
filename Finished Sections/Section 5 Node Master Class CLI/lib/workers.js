...
workers.validateCheckData = (originalCheckData)=>{
   ...
   if (originalCheckData.id && originalCheckData.userPhone && originalCheckData.protocol &&
      originalCheckData.url && originalCheckData.method && originalCheckData.success &&
            ...
            _logs.compress(logId, newFileId, (err)=>{
               if (!err) {
                  ...
                  _logs.truncate(logId, (err)=>{
                     if (!err) {
                        debug('Log truncated', logId);
                     } else {
                        debug('Error: could not truncate log', logId);
                     }
                  });
               } else {
...
// Compress and rotate the logs every 24 hrs
